...
class top extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('model_top');
    }

    function index() {
        $this->load->model('model_user');
        $data['action'] = explode('|', $this->model_user->getAction($this->session->userdata('id'), "top"));
        $this->load->view('top/view', $data);
    }
    ...
    function get() {
        ...
        $query = "select * from top where true ";
        ...
        echo $this->model_top->get($page, $rows, $query);
    }
    ...
    function get_remote_data() {
        ...
        echo $this->model_top->get_remote_data($query);
    }
    ...
    function update($code) {
        ...
        if ($this->model_top->update($data, array("code" => $code))) {
            echo json_encode(array('success' => true));
        } else {
            echo json_encode(array('msg' => $this->db->_error_message()));
        }
    ...
    function delete() {
        $code = $this->input->post('code');
        if ($this->model_top->delete(array("code" => $code))) {
            echo json_encode(array('success' => true));
        } else {
            echo json_encode(array('msg' => $this->db->_error_message()));
        }
